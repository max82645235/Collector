...
Class DB {
    ...
    //批量插入
    public function multi_insert($table,$insertfield,$dataArray) {
        $value = "";
        $sql = '';
        if( !is_array($dataArray) || count($dataArray)<=0) {
            $this->halt('没有要插入的数据');
        ...
        foreach($dataArray as $key=>$dataArrayTemp)
        {
            while(list($key,$val)=each($dataArrayTemp)) {
                $value .="'$val',";
            }
            $value = substr( $value,0,-1);
            $sql = $sql . "($value),";
        ...
        $sqlfirst = "insert into $table($insertfield) values";
        $sql = $sqlfirst . substr($sql,0,strlen($sql)-1);

        $this->write_log("批量插入 ".$sql);
        if(!$this->query($sql)) return false;
        return true;
    }
